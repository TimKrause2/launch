...
static void rv2os(aVector3d &r, aVector3d &v, double mu,
                  adouble &a, aVector3d &e, aVector3d &h);
            ...
            int iphase, Workspace* workspace)
{
    ...
    if(iphase==1){
        Eigen::Map<Eigen::Matrix<adouble, 9, 1>> ev(e);
        ...
        aVector3d h;
        adouble a;
        rv2os(r, v, md->mu, a, ecc, h);
        aVector3d ecc0 = md->ecc_v.cast<adouble>();
        adouble de = (ecc0 - ecc).norm();
        aVector3d n_h = md->n_h.cast<adouble>();

        ev.segment<6>(0) = xi;
        ev.segment<1>(6) << a;
        ev.segment<1>(7) << de;
        ev.segment<1>(8) << n_h.dot(h.normalized());
    }else if(iphase == 2){
        ...
        Eigen::MatrixXd &time) // phase 1 time
{
    ...
    double a_min = data->a/1.01;
    double a_max = data->a*1.01;
    double de = data->ecc_v.norm()*0.1;
    double hdot_min = cos(7.0*M_PI/180.0);
    ...
    Eigen::Matrix<double, 9, 1> events1upper,events1lower;
    events1upper.segment<3>(0) = data->r_vehicle0;
    events1upper.segment<3>(3) = data->v_vehicle0;
    events1lower = events1upper;
    events1lower.segment<1>(6) << a_min;
    events1lower.segment<1>(7) << 0.0;
    events1lower.segment<1>(8) << hdot_min;

    events1upper.segment<1>(6) << a_max;
    events1upper.segment<1>(7) << de;
    events1upper.segment<1>(8) << 1.0;
                  ...
                  adouble &a, aVector3d &e, aVector3d &h)
{
    ...
    e = vxh/mu - r.normalized();
    adouble p = h.squaredNorm()/mu;
    adouble e2 = e.squaredNorm();
    a = p/(1-e2);
}
