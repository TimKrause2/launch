            ...
            int iphase, Workspace* workspace)
{
    ...
    if(iphase==1){
        ...
        Eigen::Map<Eigen::Matrix<adouble, 1, 1>> ev(e);
        Eigen::Map<Eigen::Matrix<adouble, 6, 1>> xf(final_states);
        aVector3d r_vehicle = xf.segment<3>(0);
        adouble mag_r = r_vehicle.norm();
        ev << mag_r;
    }
}
        ...
        Eigen::MatrixXd &time) // phase 1 time
{
    ...
    events1upper.segment<5>(6) << data->a, data->e, data->i, data->Om, data->om;

    events1lower.segment<3>(0) = data->r_vehicle0;
    events1lower.segment<3>(3) = data->v_vehicle0;
    events1lower.segment<5>(6) << data->a, data->e, data->i, data->Om, data->om;
    ...
    problem.phases(1).bounds.lower.EndTime = data->T1 * 0.5;
    problem.phases(1).bounds.upper.EndTime = data->T1 * 2.0;

    problem.phases(2).bounds.lower.events << data->r_target0.norm();
    problem.phases(2).bounds.upper.events << data->r_target0.norm();
